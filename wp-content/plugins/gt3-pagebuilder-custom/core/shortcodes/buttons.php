	...
	public function register_shortcode($shortcodeName) {
		function shortcode_button($atts, $content = null) {
			...
			if ($icon == "") {
				...
				$button_ico = '<span class="ico_cont"><i class="ico_this '.$icon.'"></i></span>';
			}
...
if (is_array($GLOBALS["pbconfig"]['all_available_custom_buttons'])) {
    foreach ($GLOBALS["pbconfig"]['all_available_custom_buttons'] as $buttonclass => $buttonCaption) {
        $gt3pb_compileShortcodeUI .= "<option value='".$buttonclass."'>".$buttonCaption."</option>";
    }
}
...
<div class='social_icons all_available_font_icons'>
	...
	<ul class='icons_list ".$shortcodeName."_icon'>";
        foreach ($GLOBALS["pbconfig"]['all_available_font_icons'] as $icon) {
            $gt3pb_compileShortcodeUI .= "<li class='stand_social'><i data-icon-code='" . $icon . "' class='stand_icon " . $icon . "'></i></li>";
        }		
$gt3pb_compileShortcodeUI .= "
...
shortcodesUI::getInstance()->add('shortcode_button', array("name" => $shortcodeName, "caption" => "Custom Buttons", "handler" => $gt3pb_compileShortcodeUI));
