        ...
        function shortcode_pricetable_item($atts, $content = null)
        {
            ...
            if (isset($price_features) && is_array($price_features)) {
                foreach ($price_features as $value) {
                    $compile .= '<div class="price_item_text"><i class="icon-ok"></i> '.$value.'</div>';
                }
            }

$compile .= '</div><div class="price_item_btn">'.do_shortcode('[custom_button style="btn_normal btn_type5" icon="icon-download-alt" target="_self" href="'.$block_link.'"]'.$get_it_now_caption.'[/custom_button]').'</div>
						</div>
                    </div>
                </div>
            ';
