...
if (is_array($GLOBALS["pbconfig"]['all_available_quote_types'])) {
    foreach ($GLOBALS["pbconfig"]['all_available_quote_types'] as $quotetype => $quoteCaption) {
        $gt3pb_compileShortcodeUI .= "<option value='".$quotetype."'>".$quoteCaption."</option>";
    }
}
$gt3pb_compileShortcodeUI .= "
            </select>
...
shortcodesUI::getInstance()->add('blockquote', array("name" => $shortcodeName, "caption" => "Blockquote", "handler" => $gt3pb_compileShortcodeUI));
