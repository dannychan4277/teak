...
if (is_array($GLOBALS["pbconfig"]['all_available_social_icons_type'])) {
    foreach ($GLOBALS["pbconfig"]['all_available_social_icons_type'] as $socialType => $socialCaption) {
        $gt3pb_compileShortcodeUI .= "<option value='".$socialType."'>".$socialCaption."</option>";
    }
}
...
shortcodesUI::getInstance()->add('social_icon', array("name" => $shortcodeName, "caption" => "Icon", "handler" => $gt3pb_compileShortcodeUI));
