...
add_filter('the_content', 'gt3pb_add_pb_to_content');
...
if (!function_exists('gt3pb_smarty_modifier_truncate')) {
    function gt3pb_smarty_modifier_truncate($string, $length = 80, $etc = '... ',
...
#Get module html
add_action('wp_ajax_get_module_html', 'gt3pb_get_module_html');
if (!function_exists('gt3pb_get_module_html')) {
    function gt3pb_get_module_html()
    {
        ...
        echo gt3pb_get_pb_module($module_name, $module_caption, gt3pb_get_unused_id(), "", $module_size, $size_caption, $tinymce_activation_class);
...
function gt3pb_the_select_options($option_array, $selected_value = "")
{
...
function gt3pb_get_select_options($option_array, $selected_value = "")
{
    if (!isset($compile)) {
        $compile = '';
    }
...
function gt3pb_get_select_options_with_caption($option_array, $selected_value = "")
{
    if (!isset($compile)) {
        $compile = '';
    }
...
if (!function_exists('gt3pb_get_selected_pf_images')) {
    function gt3pb_get_selected_pf_images($gt3_theme_pagebuilder)
    {
...
function gt3pb_get_html_all_available_pb_modules($gt3pb_modules)
{
    if (!isset($compile)) {
        $compile = "";
    }
    if (is_array($gt3pb_modules)) {
        foreach ($gt3pb_modules as $module_key) {
            $compile .= "
...
#Custom paging
function gt3pb_get_plugin_pagination($range = 10, $type = "")
{
    $compile = "";
    if ($type == "show_in_shortcodes") {
        global $paged, $gt3pb_wp_query_in_shortcodes;
        $wp_query = $gt3pb_wp_query_in_shortcodes;
    } else {
...
if (!function_exists('gt3pb_showPortCats')) {
    function gt3pb_showPortCats($post_type_terms = "")
    {
        if (!isset($term_list)) {
            $term_list = '';
        }
        ...
        if ($count > 0) {
            ...
            if ($count > 1) {
                ...
                $term_list .= '<a href="' . $permalink . '" data-category="*">' . __('All Stuff', 'gt3_builder') . '</a>
				</li>';
            }
            ...
            if (is_array($terms)) {
                foreach ($terms as $term) {
                    ...
                    $term_list .= '><a data-category=".'. $tempname .'" href="' . $permalink . '" title="'.__('View all post filed under', 'gt3_builder').' ">' . $term->name . '</a>
                </li>';
...
if (!function_exists('gt3pb_showPortCatsMasonry')) {
    function gt3pb_showPortCatsMasonry($post_type_terms = "")
    {
        if (!isset($term_list)) {
            $term_list = '';
        }
        ...
        if ($count > 0) {
            ...
            $term_list .= '<a class="' . (!isset($_GET['slug']) ? 'selected' : '') . '" href="#filter" data-category="*">' . __('All Stuff', 'gt3_builder') . '</a>
			</li>';
            $termcount = count($terms);
            if (is_array($terms)) {
                foreach ($terms as $term) {
                    ...
                    $term_list .= '><a href="#filter" data-category=".' . $tempname . '" title="'.__('View all post filed under', 'gt3_builder').' ">' . $term->name . '</a>
                </li>';
                    if ($count != $i) $term_list .= ' '; else $term_list .= '';
            ...
            return '<ul class="optionset isotope_sorting" data-option-key="filter">' . $term_list . '</ul>';
        }
    }
}

function gt3pb_pre($array)
{
    echo "<gt3pb_pre>Here your gt3pb_pre: ";
    print_r($array);
    echo "</gt3pb_pre>";
}


function gt3pb_my_custom_admin_head()
{
...
add_action('admin_head', 'gt3pb_my_custom_admin_head');
